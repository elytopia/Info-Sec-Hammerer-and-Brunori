...
# Reshape into tf.image.encode_jpeg format
            images = tf.reshape(tf.cast(adv_observations, tf.uint8), [84, 84, 0])

# Encode
            images_encode = tf.image.encode_jpeg(images)

# Create a files name
            fname = tf.constant('save.jpeg')
...
# Write files 
            ...
            #image_data = tf.image.convert_image_dtype(adv_observations[:,:,:,0], dtype= tf.uint8)
            #adv_x_saved = tf.image.encode_jpeg(image_data)#, quality=100, format='grayscale')
            #writer = tf.write_file('test_sinogram.jpg', adv_x_saved)
            #sess.run(writer)
            #np.save('adv_x.npy', adv_observations[:,:,:,0])#, allow_pickle = False)
        ...
        if attack != None:
            if attack == 'fgsm':
             ...
             #   adv_observations = adversary.generate(observations_ph.get(), eps=1.0/255.0,
                ...
                #def wrapper(x):
                #    return q_func(x, num_actions, scope="q_func", reuse=True)
                adversary = CarliniWagnerL2(q_func(obs_tp1_input.get(), num_actions, scope="q_func", reuse=True, concat_softmax=True, noisy=noisy), sess=U.get_session())
                cw_params = {'binary_search_steps': 1,
                             'max_iterations': 100,
                             'learning_rate': 0.1,
                             'initial_const': 10,
                             'clip_min': 0,
                             'clip_max': 1.0}
                adv_observations = adversary.generate(obs_tp1_input.get(), **cw_params) * 255.0
